    ...
    <div className="space-y-8">
      ...
      {flagged.length > 0 && (
        <section className="card p-6">
          <SectionHeader
            title={
              <span className="flex items-center gap-2">
                <Flag size={14} className="text-red" /> Flagged by Users
              </span>
            }
          ...
          <SubmissionList
            items={flagged}
            showFlags
            onAct={(id, action) => act(id, action, 'flagged')}
            disabled={isPending}
          />
        </section>
      )}
      ...
      <section className="card p-6">
        ...
        {pending.length === 0 ? (
          ...
          <SubmissionList
            items={pending}
            onAct={(id, action) => act(id, action, 'pending')}
            disabled={isPending}
          />
        )}
      </section>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-text-muted">
                      Day {s.day_number} · {DOMAIN_LABELS_SHORT[s.domain as LefDomain]}
                    </span>
                  </div>
...
const ACTION_CLASSES: Record<ActionBtnProps['variant'], string> = {
  ...
  ghost:
    'border-[var(--border-subtle)] text-text-secondary hover:border-success hover:text-success',
};
